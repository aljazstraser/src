...
public class MoviesServiceImpl implements MoviesService {
    ...
    public MovieDto read(Long id, boolean includeActors) {
        ...
        if (includeActors) {
            movieDto.setActors(actorMovieClientHelper.getMovieActors(movieDto));
        }
        return movieDto;
    }
    ...
    public void update(Long id, MovieDto movieDto) {
        ...
        if (ObjectUtils.notEqual(id,movieDto.getId())) {
            throw new MovieValidationException(String.format("Request id %s is not matching body id: %s", id, movieDto.getId()));
        }
