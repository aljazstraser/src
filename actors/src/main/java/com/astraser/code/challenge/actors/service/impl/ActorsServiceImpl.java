...
public class ActorsServiceImpl implements ActorsService {
    ...
    public ActorDto read(Long id, boolean includeMovies) {
        ...
        if (includeMovies) {
            actorDto.setMovies(actorMovieClientHelper.getActorMovies(actorDto));
        }
        return actorDto;
    }

    @Override
    public void update(Long id, ActorDto actorDto) {
        logger.info("Updating actor with id {}: ", id);

        if (ObjectUtils.notEqual(id,actorDto.getId())) {
